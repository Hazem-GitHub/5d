"use strict";
export function magicMouse(options) {
  options = options || {};
  ...
  if ("disable" != options.cursorOuter) {
    if(document.getElementById("magicMouseCursor")){
      document.getElementById("magicMouseCursor").remove();
    }
  ...
  if (!options.defaultCursor) {
    if(document.getElementById("magicPointer")){
      document.getElementById("magicPointer").remove();
    }
  ...
  document.addEventListener("mousemove", function(event) {
    ...
    setTimeout(() => {
      if (!stopFlag) {
        outerX = event.clientX - cursorDOM.offsetWidth / 2;
        outerY = event.clientY - cursorDOM.offsetHeight / 2;
      }
    }, 50);
  });
  ...
  const circleMove_mouseEnterHover = item => {
    ...
    if (cursorDOM) {
      ...
      if (item.classList.contains("magic-hover__square")) {
        ...
        outerX = elementPos.left - (0.05 * elementPos.width);
        outerY = elementPos.top - (0.05 * elementPos.height);
        // outerX = cursorDOM.getBoundingClientRect().left/2;
        // outerY = cursorDOM.getBoundingClientRect().top/2;
        cursorOuterWidth = elementPos.width * 1.1;
        cursorOuterHeight = elementPos.height * 1.1;
      } else {
        outerX = elementPos.left - (0.05* elementPos.width);
        outerY = elementPos.top - (0.05* elementPos.height);
        // cursorOuterWidth = elementPos.width;
